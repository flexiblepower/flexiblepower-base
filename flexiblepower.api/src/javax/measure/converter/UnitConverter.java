 ...
 * 
 * <p>
 * Sub-classes must ensure unicity of the {@link #IDENTITY identity} converter. In other words, if the result of an
 * operation is equivalent to the identity converter, then the unique {@link #IDENTITY} instance should be returned.
 * </p>
 * 
...
public abstract class UnitConverter implements Serializable {
    ...
    /**
     * Returns the inverse of this converter. If <code>x</code> is a valid value, then
     * <code>x == inverse().convert(convert(x))</code> to within the accuracy of computer arithmetic.
     * 
     ...
     * <code>convert(u + v) == convert(u) + convert(v)</code> and <code>convert(r * u) == r * convert(u)</code>. For
     * linear converters the following property always hold:[code] y1 = c1.convert(x1); y2 = c2.convert(x2); then y1*y2
     * = c1.concatenate(c2).convert(x1*x2)[/code]
     * 
     * @return <code>true</code> if this converter is linear; <code>false</code> otherwise.
     */
    public abstract boolean isLinear();

    /**
     * Indicates whether this converter is considered the same as the converter specified. To be considered equal this
     * converter concatenated with the one specified must returns the {@link #IDENTITY}.
     * 
     * @param cvtr
     ...
     * the specified converter, and then converting by this converter.
     * 
     * <p>
     * Note: Implementations must ensure that the {@link #IDENTITY} instance is returned if the resulting converter is
