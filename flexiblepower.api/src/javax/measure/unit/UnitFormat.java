 ...
 * 
 * <p>
 * For all {@link SI} units, the 20 SI prefixes used to form decimal multiples and sub-multiples of SI units are
 * recognized. {@link NonSI} units are directly recognized. For example:[code]
 * Unit.valueOf("m°C").equals(SI.MILLI(SI.CELSIUS)) Unit.valueOf("kW").equals(SI.KILO(SI.WATT))
...
public abstract class UnitFormat extends Format {
    ...
    /**
     * Returns the unit format for the default locale (format used by {@link Unit#valueOf(CharSequence)
    ...
    /**
     * Returns the <a href="http://aurora.regenstrief.org/UCUM/ucum.html">UCUM </a> international unit format; this
     * format uses characters range <code>0000-007F</code> exclusively and <b>is not</b> locale-sensitive. For example:
     * <code>kg.m/s2</code>
     * 
     ...
     */
    public abstract Unit<? extends Quantity>
            parseProductUnit(CharSequence csq, ParsePosition pos) throws ParseException;
     ...
     * UnitFormat.getInstance().label(DAY.multiply(365), "year"); UnitFormat.getInstance().label(METER.multiply(0.3048),
     * "ft"); [/code] If the specified label is already associated to an unit the previous association is discarded or
     * ignored.
     * 
     * @param unit
     ...
     * @throws IllegalArgumentException
     *             if the label is not a {@link UnitFormat#isValidIdentifier(String)} valid identifier.
     */
     ...
     * UnitFormat.getLocaleInstance().alias(METER.multiply(0.3048), "feet"); UnitFormat.getLocaleInstance().alias(METER,
     * "meter"); UnitFormat.getLocaleInstance().alias(METER, "metre"); [/code] If the specified label is already
     * associated to an unit the previous association is discarded or ignored.
     * 
     * @param unit
    ...
    protected static class DefaultFormat extends UnitFormat {
        ...
        static boolean isUnitIdentifierPart(char ch) {
            return Character.isLetter(ch) || (!Character.isWhitespace(ch) && !Character.isDigit(ch)
                                              && (ch != '·')
                                              && (ch != '*')
                                              && (ch != '/')
                                              && (ch != '(')
    ...
    private static final Unit<?>[] SI_UNITS = { SI.AMPERE,
                                               SI.BECQUEREL,
                                               SI.CANDELA,
                                               SI.COULOMB,
                                               SI.FARAD,
    ...
    private static final String[] PREFIXES = { "Y",
                                              "Z",
                                              "E",
                                              "P",
                                              "T",
    ...
    private static final UnitConverter[] CONVERTERS = { E24,
                                                       E21,
                                                       E18,
                                                       E15,
                                                       E12,
    ...
    static {
        ...
        for (int i = 0; i < PREFIXES.length; i++) {
            ...
            DEFAULT.label(SI.KILOGRAM.transform(CONVERTERS[i].concatenate(Em3)), PREFIXES[i] + "g");
            if (PREFIXES[i] == "µ") {
